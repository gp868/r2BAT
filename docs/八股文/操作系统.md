...
[GDB入门教程之如何使用GDB启动调试_牛客博客 (nowcoder.net)](https://blog.nowcoder.net/n/755bcf3ca85145d3b10bd7246462f76c#GDB_37)

[GDB调试_牛客博客 (nowcoder.net)](https://blog.nowcoder.net/n/dfcd44e2d36a409fa31b29e8c08cc3fb)
...
2. 启动一个汇编代码例程**保存通用寄存器及其它易失性信息**，以免被操作系统破坏。这个例程将操作系统作为一个函数来调用。
...
5. 页面调度（*也就是算法过程，看算法整理笔记*）
   1. 操作系统查找一个**空闲的页框(物理内存中的页面)，如果没有空闲页框则需要通过页面置换算法找到一个需要换出的页框。**
...
[linux下把进程/线程绑定到特定cpu核上运行_guotianqing的博客-CSDN博客_进程绑核](https://blog.csdn.net/guotianqing/article/details/80958281)

- 对于使用tcp通信的两端，如果client已经退出，此时服务端继续send会出现什么问题？

在linux下编写TCP socket程序时，如果客户端突然退出，导致连接中断，这个时候服务端如果继续调用send函数发送数据的话，会导致整个进程退出，这是我们不愿看到的。(注：如果是服务端突然退出，客户端继续调用send发送，是不会导致进程退出的)。当服务端尝试使用一个未连接的socket进行send数据时，会抛出一个SIGPIPE信号，这个信号的缺省处理方法是退出进程。

比较简单的方法有以下2种：

1. 重新定义一个信号处理函数，覆盖系统默认处理方法
