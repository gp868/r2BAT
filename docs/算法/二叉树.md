...
这题和 ⌈[102. 二叉树的层序遍历](https://leetcode-cn.com/problems/binary-tree-level-order-traversal)⌋ 类似，只是要控制遍历方向，使用数组的**深度奇偶变化**来控制遍历顺序。
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204201830207.jpeg" alt="img" style="zoom:60%;" />
...
何为最近公共祖先呢？**如果一个节点能够在它的左右子树中分别找到`p`和`q`，则该节点为`LCA`节点**。
...
public:
    ...
    string serialize(TreeNode* root) {
        if(!root) return "#"; // 易hu'lue
        string left = serialize(root->left);
        string right = serialize(root->right);
        return right + ',' + left + ',' + to_string(root->val);
        // 注意要按照右左中顺序，因为strs是从尾部开始取元素
    }
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204171510562.png" alt="image-20220417151011413" style="zoom:80%;" />
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271559371.jpeg" alt="img" style="zoom:50%;" />
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271942183.jpeg" alt="img" style="zoom:50%;" />
...
给定一个单链表的头节点  `head` ，其中的元素 **按升序排序** ，将其转换为高度平衡的二叉搜索树。本题中，一个高度平衡二叉树是指一个二叉树*每个节点* 的左右两个子树的高度差不超过 1。

**示例:**

<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204301023724.jpeg" alt="img" style="zoom:40%;" />
...
public:
    ...
    TreeNode* build(ListNode* &p, int start, int end){
        if(start > end) return nullptr;
        int mid = (start + end + 1) / 2;
        TreeNode* leftTree = build(p, start, mid - 1);
        TreeNode* root = new TreeNode(p->val);
        p = p->next;
        TreeNode* rightTree = build(p, mid + 1, end);
        root->left = leftTree;
...
public:
    bool verifyPostorder(vector<int>& postorder) {
        ...
        return get(postorder, 0 , n - 1);
    }

    bool get(vector<int>& postorder, int lo, int hi){
        if(lo > hi) return true;
        int i = lo;
        while(i < hi && postorder[i] < postorder[hi]){
            i++;
        }
        int mid = i;
        while(i < hi && postorder[i] > postorder[hi]){
        ...
        bool left = get(postorder, lo, mid - 1);
        bool right = get(postorder, mid, hi - 1);
        return left && right && i == hi;
    }
};
```
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204251606314.jpeg" alt="img" style="zoom:50%;" />
...
中序遍历，需要比较节点和上一个节点的大小关系，所以引入指针`pre`。
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204301210130.png" alt="Picture1.png" style="zoom:50%;" />
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204270954033.jpeg" alt="img" style="zoom:50%;" />
...
public:
    TreeNode* searchBST(TreeNode* root, int val) {
        if(!root) return root;
        if(root->val > val) return searchBST(root->left, val); //比目标值大，到左子树寻找
        if(root->val < val) return searchBST(root->right, val); //比目标值小，到右子树找
        //等于目标值，返回节点
        return root;
    }
};
...
public:
	...
	TreeNode* find(TreeNode* root, int val1, int val2){
        if(!root) return nullptr;
        if(root->val > val2) return find(root->left, val1, val2); // 当前节点太大，去左子树找
        if(root->val < val1) return find(root->right, val1,val2); // 当前节点太小，去右子树找
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271918176.jpeg" alt="img" style="zoom:40%;" />
...
public:
    ...
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        if(!root) return root;
        if(root->val < low) return trimBST(root->right, low,high); // 等于删除 root 以及 root 的左子树
        if(root->val > high) return trimBST(root->left, low, high); // 等于删除 root 以及 root 的右子树
        // 闭区间 [lo, hi] 内的节点什么都不做
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271015149.jpeg" alt="img" style="zoom:50%;" />
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271044120.png" alt="img" style="zoom:25%;" />
...
<img src="https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271044561.png" alt="img" style="zoom:48%;" />
...
![img](https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202204271046754.png)
