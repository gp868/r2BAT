...
![img](https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202205311640706.png)
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        d[0][0] = - prices[0];
        for(int i = 1; i < n; i++){
            d[i][0] = max(d[i - 1][0], max(d[i - 1][2], d[i - 1][3]) - prices[i]);
            d[i][1] = d[i - 1][0] + prices[i];
            d[i][2] = d[i - 1][1];
            d[i][3] = max(d[i - 1][2], d[i - 1][3]);
        }
        return max(d[n - 1][1], max(d[n - 1][2], d[n - 1][3]));
    }
};
```
...
![img](https://gcore.jsdelivr.net/gh/gp868/myFigures/img/202207031009179.png)
...
当 `matrix[i][j]` 为 1，且它的左边、上边、左上角都存在正方形时，`matrix[i][j]` 才能够作为一个更大的正方形的右下角。
...
相当于是这三处都能组成的正方形行的最小值，这样这三个小的正方形叠加到一起，就是一个缺少了右下角的正方形，故加1就为当前位置的最大的正方形。题目最终想要的答案就是最大边长 `max(dp[..][..])` 的平方。
...
1. 动态规划；
2. matrix[i][j]表示到当前位置的最小值，而到当前位置的方法只有从左上角，上方，右上角，三个位置跳转过来，故只需要确定这三个位置中的最小值，加上当前位置上的值，就是到当前位置的最小值；
3. 从最后一行确定最小值，即为最小和；
    ...
    int minFallingPathSum(vector<vector<int>>& matrix) {
        ...
        for(int i = 1; i < m; i++){
            for(int j = 0; j < n; j++){
                ...
                if(j > 0) num = min(num, matrix[i - 1][j - 1]);
                if(j < n - 1) num = min(num, matrix[i - 1][j + 1]);
                matrix[i][j] += num;
            }
        }
        int res = matrix.back()[0];
        for(int j = 0; j < n; j++){
            res = min(res, matrix.back()[j]);
        }
        return res;
    }
};
