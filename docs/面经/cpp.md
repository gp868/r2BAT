...
- 循环往一个vector里面插入10000个数据，一次插入一个，怎么高效，用resize()还是reserve()，为什么？

  在循环中往一个 `vector` 里插入 10000 个数据时，建议使用 `reserve()` 函数，因为它可以一次性为目标容量分配足够的内存，从而避免了多次内存重新分配和数据拷贝，因此更高效。

  `resize()` 函数会改变 `vector` 的大小，为新的大小分配内存，并初始化新添加的元素。在循环中使用 `resize()`并不理想，因为每次调用`resize()` 都会导致内存重新分配和数据拷贝，带来额外的性能开销。
  ...
  int main() {
    std::vector<int> data  data.reserve(10000);  // 为10000个元素预分配内存
  ...
  这个示例将 `vector` 的容量预留为 10000 个元素，然后通过循环使用 `push()` 不断插入元素。由于已经预分配了足够的内存，`vector` 在插入时无需重新分配内存，从而提高了性能。
